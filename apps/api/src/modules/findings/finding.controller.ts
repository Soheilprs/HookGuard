...
export async function findingController(
  app: FastifyInstance,
  options: { service: FindingService },
): Promise<void> {
  const { service } = options;

  app.get(
    '/hooks/:address/findings',
    async (
      ...
      try {
        ...
        return await service.getForHook(request.params.address, chainId);
      } catch (error) {
